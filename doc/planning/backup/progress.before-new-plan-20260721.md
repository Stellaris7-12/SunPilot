...
## 会话：2026-07-21（模块 I4 前端布局与业务逻辑重构）

### 背景

用户要求按照 `doc/planning/task_plan.md` 中模块 I4 清单完成开发。本轮只重构前端企业工单壳和前端业务派生展示，不修改后端 API、数据库 schema、Agent 编排或新增依赖。

### 执行内容

- 重构 `frontend/src/views/EnterpriseTicketShellView.vue`：形成左侧/中间业务主工作区 + 右侧 SunPilot 智能辅助区的双栏叙事。
- 将 `Agent Copilot` 默认展示文案改为 `SunPilot`，主按钮改为业务动词：生成处理建议、填入回单、定位证据、请求补充、进入复核、打开系统审计。
- 将处理日志、证据链、字段补全/信息核验、系统审计从主业务表单迁移到右侧 SunPilot 可展开卡片；默认展开当前建议和状态流转，技术明细默认折叠。
- 扩展 `frontend/src/utils/business.ts`：新增字段核验项和回单工作区派生数据，按业务语言展示原始字段、自动补全字段、来源系统、证据编号、缺失兜底和风险复核。
- 将主详情页“字段与风险核验”重构为“处理依据/信息核验”摘要，不再把工具名和技术参数暴露在普通坐席主路径。
- 将回单区拆成多窗口工作区：客户回单、内部处理意见、复核摘要、客户追问、跟进计划、证据附件、模板选择。
- 增加回单业务按钮：套用模板、填入 SunPilot 建议、插入证据编号、生成客户追问/内部说明、保存草稿、提交复核并结案。
- 增加草稿状态提示：未生成、已填入、坐席已编辑、可结案、已提交；坐席已编辑时，SunPilot 填充采用追加方式，避免直接覆盖已有草稿。
- 将首页技术口径收敛：`REST/SSE` 改为业务化“智能服务已连接”，`工具命中率` 改为“业务能力匹配率”；技术细节保留到右侧系统审计。
- 更新 `doc/planning/task_plan.md`：模块 I4 标记为已完成，开发方案 checklist 全部勾选。

### 验证结果

- `cd frontend && npm.cmd run build`：通过。
- 文本扫描确认前端企业壳普通路径不再出现 `Agent Copilot`、`REST/SSE`、`工具命中率`、旧“字段与风险核验”等展示文案。

### 当前结论

- I4 前端布局与业务逻辑重构已落地；业务系统主区可独立处理工单，右侧 SunPilot 承载智能建议、状态流转、日志、证据、字段补全和系统审计。
- 后续进入 I5 时，应围绕低风险自动成功、中风险人工确认、高风险查询/建案/协办三条演示链路做端到端联调和演示数据复位验证。
