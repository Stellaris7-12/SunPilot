...
- 模块 I 改为串行推进，不再设置并行开发策略。执行顺序按依赖关系确定：先数据结构与 MySQL/TDSQL 底座，再 CRUD 和状态流转，再 Mock Tools 生产化，最后做前端布局与逻辑重构。
...
目标：在后端字段、CRUD 和 Mock 工具稳定后，重构前端为真实业务人员可理解的企业工单处理台。页面形成“左侧业务系统 + 右侧智能辅助区”的双栏叙事，并把回单从单文本框升级为多组件工作区。

开发方案：
- [x] 调整主布局：业务系统主工作区固定在左侧或中间偏左，承载查询、队列、工单详情、客户/卡片/交易/权益信息、业务按钮和回单编辑；Agent 侧边栏固定在右侧，承载智能建议、状态流转、处理日志、证据、字段补全和技术审计入口。
- [x] 将 `Agent Copilot` 默认文案改为`SunPilot`，主操作按钮使用业务动词：生成处理建议、填入回单、定位证据、请求补充、进入复核。
- [x] 首屏优先展示查询条件、队列列表、工单详情、客户/卡片/交易/权益信息、回单和业务状态流转；处理日志从主表单中移到 Agent 右侧栏，以可展开卡片展示。
- [x] Agent 右侧栏采用可展开卡片：当前建议、状态流转、处理日志、证据链、字段补全、系统审计。默认展开“当前建议”和“状态流转”，折叠技术审计。
- [x] 增加动态数据/状态流转展示：启动处理后展示从接单提取、字段补全、业务核验、执行/建案、回单生成、待复核/结案的步骤推进；每一步显示输入、输出、状态变化、证据编号和下一步责任人。
- [x] 增强点击交互：队列点击切换详情，状态节点点击定位到相关数据卡片，证据点击展开来源，回单按钮点击填入对应子窗口，日志卡片可展开查看前后状态差异。
- [x] 将 `REST/SSE`、`工具命中率`、`真实链路测评摘要`、`Agent Trace`、工具请求参数等技术信息默认折叠到“系统审计/技术明细”。
- [x] 新增业务查询和表格列配置：工单号、客户号、客户姓名、分类、优先级、状态、处理人、到期时间、最近处理记录。
- [x] 详情页按真实工单系统分区：基本信息、客户信息、卡片信息、关联交易/权益/申请、发单内容、回单工作区、附件/备注占位；处理日志和审计明细放到右侧 Agent 区。
- [x] 移除或重构“字段与风险核验”：不再作为含义模糊的独立表格展示；改为“处理依据/信息核验”卡片，只展示对当前处理有解释价值的字段、来源系统、补全方式、证据编号和风险结论。
- [x] 增加字段补全展示区：显示“原始工单字段、自动补全字段、补全来源、证据编号、是否仍需人工确认”，让业务人员知道哪些信息来自跨系统查询；若没有补全或核验价值则不展示空表。
- [x] 将 `pending_info` 文案改为最后兜底状态：只有受控工具无法补齐且无法安全推断时，才展示为“待客户补充”。
- [x] 保留技术审计入口，但不在普通坐席路径中主动突出 Agent 编排细节。
- [x] 将回单区拆成多个子组件/子窗口：客户回单、内部处理意见、复核摘要、客户追问、跟进计划、证据附件、模板选择。
- [x] 增加模板和填空槽位：如处理结果、证据编号、到账时间、权益有效期、申请节点、交易金额、商户名称、下一步责任人。
- [x] 增加业务按钮：套用模板、填入 Agent 建议、插入证据编号、生成客户追问、生成内部备注、复制回单、保存草稿、提交复核、结案。
- [x] 支持不同场景的回单结构：优惠券补发显示补发结果和查收路径；权益查询显示资格、次数和有效期；交易争议显示查询结果、建案/协办建议和材料提示；资料变更显示人工确认和生效说明。
- [x] 回单工作区与右侧 Agent 建议联动：点击 Agent 侧栏建议时，只填入对应子窗口或槽位，不直接覆盖坐席已编辑内容。
- [x] 增加草稿状态提示：未生成、已填入、坐席已编辑、待复核、可结案、已提交。

验收标准：
- 首屏观感接近业务工单系统，不再像独立 AI Demo 页。
- 页面布局能清晰讲出“业务系统在左，智能辅助在右”；关闭或折叠右侧栏后，业务系统仍可独立查看和处理工单。
- 处理日志、证据链和技术审计不再挤占主业务表单，而是在右侧栏中以可展开卡片呈现。
...
目标：在 I1-I4 串行完成后做稳定性收口，确保 MySQL/TDSQL、CRUD、Mock Tools、Agent 处理链路和前端业务页面形成可复现演示。

开发方案：
- [ ] 串行联调 I1-I4：先确认数据库和 Repository，再确认 CRUD，再确认工具与补全，最后确认前端页面。
- [ ] 准备低风险自动成功、中风险人工确认后成功、高风险查询/建案/协办三条核心演示链路。
- [ ] 准备演示数据重置脚本和说明，确保每次演示前可恢复干净数据。
- [ ] 更新启动文档、演示脚本、数据库配置说明和 Mock 工具说明。
- [ ] 运行后端 compileall、模块 smoke、MySQL/TDSQL 数据库 smoke、前端 build。
- [ ] 检查旧 SQLite 测试污染数据不会进入 MySQL/TDSQL 主演示库。

模块 I 总体验收标准：
- 系统具备准生产业务沙盘感：数据可信、分类丰富、CRUD 可用、工具调用有真实业务规则。
- 主演示数据库使用 MySQL 或 TDSQL，SQLite 仅作为测试兼容或 fallback。
...
- 修改后至少运行后端 compileall、相关 smoke、MySQL/TDSQL 数据库 smoke，以及 `cd frontend && npm run build`。
...
| 阶段 8 | 准生产业务沙盘升级 | MySQL/TDSQL 主库、重构数据结构、干净业务种子库、CRUD、生产化 Mock 工具、业务化前端 |
