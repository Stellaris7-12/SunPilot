...
目标：把当前系统收束为更接近真实信用卡二线工单处理台的准生产沙盘。模块 I 只保留四条主线：业务数据基座、工单流转、工具与补全、页面演示。所有细节只保留能指导实现的最小信息，避免各子模块之间重复。

设计原则：
- 业务演示数据、评测样本和运行数据分离。
- 工具优先覆盖成功路径和可复用证据，失败只保留必要边界。
- 缺字段先尝试受控只读补全，再进入人工补充或确认。
- 前端默认面向业务坐席，技术审计默认折叠。

### I1：数据基座

目标：建立可重置的干净业务库，替换 SQLite 演示感，保证工单、客户和状态口径可信。

开发方案：
- [ ] 迁移演示数据库到 MySQL 或 TDSQL，保留本地 fallback 仅用于开发兼容。
- [ ] 补齐工单核心字段：`customer_id`、`category`、`subcategory`、`priority`、`channel`、`assignee`、`department`、`due_at`、`updated_at`、`closed_at`、`final_reply`。
- [ ] 建立 Mock 业务域数据：客户档案、卡片账户、交易流水、活动/优惠券、权益、申请单、历史工单。
- [ ] 重建业务种子数据，覆盖 30-50 条可信工单和主要信用卡二线场景。
- [ ] 提供受控重置脚本或命令，能重置演示库但不影响评测样本。
- [ ] 明确区分业务种子、评测样本、Mock 业务域数据和运行中产生的处理记录。

验收标准：
- `/api/tickets` 不再出现测试污染工单。
- 前端客户号、工单号、分类、状态和处理人展示一致。
- 至少 10 个二级业务分类有可展示工单，列表不再大面积为空。
- MySQL/TDSQL 可作为主演示数据库，SQLite 只作为 fallback。

### I2：工单流转

目标：补齐最小 CRUD 和状态流转，让前端像业务系统而不是只读 Demo。

开发方案：
- [ ] 补齐列表查询条件：工单号、客户号、客户姓名、分类、状态、优先级、处理人、创建时间、SLA。
- [ ] 补齐写入接口：新建、编辑、指派、作废、重开、保存草稿、结案。
- [ ] 保留结案门禁，仍只能通过 `/close` 完成最终结案。
- [ ] 增加操作日志，区分坐席动作、系统动作和智能辅助动作。
- [ ] 将状态流转限制在状态机内，避免任意状态直接跳转。
- [ ] 前端提供对应按钮和禁用原因，不允许只靠接口能力存在。

验收标准：
- 至少能完成新建、查看、编辑、指派、重开、结案的最小闭环。
- 状态流转不允许绕过状态机直接跳结案。
- 每次写操作都有日志，能看到操作者、动作、前后状态和时间。

### I3：工具与补全

目标：把 Mock 工具做成可审计的业务模拟层，优先提升成功路径和信息补全能力。

开发方案：
- [ ] 形成三类工具：信息补全、业务核验、受控执行；首批至少 10 个工具可演示。
- [ ] 信息补全工具覆盖客户查询、卡片状态、历史工单、活动规则和交易定位。
- [ ] 业务核验工具覆盖活动达标、券状态、权益资格、申请进度、账单/交易明细。
- [ ] 受控执行工具覆盖优惠券补发、积分修复申请、年费减免申请、争议建案、资料变更确认后执行。
- [ ] 让工具先查业务域数据，再返回结果，不再只套固定响应。
- [ ] 增加幂等、权限门禁、证据编号和审计日志。
- [ ] 把缺字段补全前置到只读查询，减少直接 `pending_info`。
- [ ] 成功路径优先覆盖：活动达标未到账、权益可用、申请单存在、交易可定位、确认后资料变更成功。

验收标准：
- 低风险场景可稳定成功并生成证据。
- 缺字段时优先补全，不是默认人工介入。
- 工具调用结果可审计、可复用、可回放。
- 重复补发返回已有流水和证据，不制造重复业务结果。
- 高风险或写入类动作进入确认/建案/协办，不包装成自动结案。

### I4：页面演示

目标：把页面组织成“左业务、右 Agent”的双栏结构，并提升动态流转感和回单表达能力。

开发方案：
- [ ] 业务系统固定在左侧，Agent 侧边栏固定在右侧。
- [ ] 左侧业务区包含查询、队列、工单详情、客户/卡片/交易/权益信息、业务按钮和回单工作区。
- [ ] 右侧 Agent 区包含当前建议、状态流转、处理日志、证据链、字段补全和系统审计卡片。
- [ ] 处理日志、证据链、字段补全和系统审计收进右侧可展开卡片，技术审计默认折叠。
- [ ] 增加动态状态流转展示，让处理步骤、状态变化和下一责任人可见。
- [ ] 将“字段与风险核验”重构为“处理依据/信息核验”，只保留有解释价值的字段。
- [ ] 把回单区拆成多组件：客户回单、内部备注、复核摘要、证据附件、模板与填空槽位。
- [ ] 增加交互动作：点击状态节点定位相关卡片，点击证据展开来源，点击 Agent 建议局部填入回单槽位。

验收标准：
- 页面能一眼看出业务系统与 Agent 的分工。
- 点击、展开、填充和状态变化在演示中是可见的。
- 回单区不再只有单一文本框。
- 普通业务路径不需要理解 Agent、Trace、SSE 或工具参数。
- 评委能看到数据从原始工单到补全、核验、执行/建案、回单建议和状态变化的过程。
...
主 Agent：
- [ ] 负责接口契约、模块边界、最终合并和验证。
- [ ] 控制 MySQL/TDSQL、状态机、工具审计和前端展示口径一致。

子 Agent A：
- [ ] 负责数据基座、工单流转、工具层和后端 smoke。
- [ ] 优先交付稳定字段契约和可调用工具，供前端并行适配。

子 Agent B：
- [ ] 负责页面布局、交互、回单区和技术隐藏。
- [ ] 基于子 Agent A 的字段契约适配左业务右 Agent 的页面。

模块 I 总体验收标准：
- 主演示数据库切换到 MySQL 或 TDSQL。
- 数据、工单、工具、页面四条主线边界清晰，没有重复职责。
- 前端能展示真实业务流转，而不是堆满技术面板。
